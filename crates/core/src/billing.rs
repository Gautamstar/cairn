...
pub enum BillingEvent {
    ...
    SubscriptionEnded { subscription: String },
    Ignored,
}
        ...
        Some(match envelope.kind.as_str() {
            "checkout.session.completed" => {
                ...
                match (
                    ...
                    (true, Some(reference), Some(payment_link), Some(subscription), Some(customer)) => {
                        Self::CheckoutCompleted { reference, payment_link, subscription, customer }
                    }
                    _ => Self::Ignored,
                }
            }
...
mod tests {
    ...
    fn an_old_signature_is_a_replay() {
        assert!(!verify_signature(&header(SIG), BODY, SECRET, T + SIGNATURE_TOLERANCE_SECS + 1));
        assert!(!verify_signature(&header(SIG), BODY, SECRET, T - SIGNATURE_TOLERANCE_SECS - 1));
    }
    ...
    fn malformed_headers_fail_closed() {
        for bad in ["", "garbage", "v1=abc", &format!("t=notanumber,v1={SIG}"), &format!("t={T}"), &format!("t={T},v1=zz")] {
    ...
    fn a_cancellation_is_read() {
        ...
        assert_eq!(
            BillingEvent::parse(body),
            Some(BillingEvent::SubscriptionEnded { subscription: "sub_9".into() })
        );
    }
    ...
    fn the_plan_comes_from_the_link_stripe_charged() {
        assert_eq!(plan_for_link("plink_s", "plink_s", "plink_p"), Some(Plan::Starter));
        assert_eq!(plan_for_link("plink_p", "plink_s", "plink_p"), Some(Plan::Pro));
